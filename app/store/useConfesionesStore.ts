...
type Actions = {
  addPendiente: (c: { content: string; category: Category; imageUri?: string }) => void;
...
export const useConfesionesStore = create<State & Actions>((set, get) => ({
  ...
  aprobadas: [],
  likedIds: [],
  addPendiente: ({ content, category, imageUri }) => {
    const id = Math.floor(Math.random() * 1_000_000) + 1000;
    const nuevo: Confesion = { id, content, category, date: Date.now(), likes: 0, imageUri, nexo: "Anon" };
  ...
  toggleLike: (id) => {
    ...
    set((s) => ({
      ...
      aprobadas: s.aprobadas.map((c) => (c.id === id ? { ...c, likes: c.likes + (has ? -1 : 1) } : c))
    }));
  },
