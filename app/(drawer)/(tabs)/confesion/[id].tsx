        ...
        <View style={[styles.card, { backgroundColor: colors.surface }, cardShadow]}>
          ...
          <View style={styles.statusRow}>
            <View
              style={[
                styles.statusBadge,
                { borderColor: statusTint, backgroundColor: `${statusTint}22` },
              ]}
            >
          ...
          {(confesion.approvedAt || confesion.rejectedAt || confesion.rejectionReason) && (
            <View style={styles.metaContainer}>
              {confesion.approvedAt && (
                <Text style={[styles.metaText, { color: colors.subtle }]}>
                  Aprobada el {formatTimestamp(confesion.approvedAt)} por{" "}
                  {confesion.approvedBy ?? "—"}
                </Text>
              )}
              ...
              {!!confesion.rejectionReason && (
                <Text style={[styles.metaText, styles.metaReason]}>
                  Motivo: {confesion.rejectionReason}
                </Text>
              )}
            </View>
          )}
          ...
          {moderationLogs.length > 0 && (
            <View style={styles.logsContainer}>
              <Text style={[styles.logsTitle, { color: colors.subtle }]}>
                ...
                .map((log, index) => (
                  ...
                  >
                    <Text
                      style={[
                        styles.logAction,
                        { color: log.action === "approved" ? "#30d158" : "#ff3b30" },
                      ]}
                    >
                    ...
                    {log.user?.name && (
                      <Text style={[styles.logMeta, { color: colors.subtle }]}>
                        Por: {log.user.name}
                      </Text>
                    )}
                    {!!log.reason && (
                      <Text style={[styles.logMeta, styles.metaReason]}>
                        Motivo: {log.reason}
                      </Text>
                    )}
                  </View>
...
const styles = StyleSheet.create({
  ...
  category: { fontSize: 13, fontWeight: "600", textTransform: "uppercase" },
